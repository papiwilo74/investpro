...
class TestTradingBrain:
    ...
    def test_entry_score_below_threshold(self):
        ...
        decision = brain.decide(df=df, score=0.1, has_position=False)
        assert decision.action == "HOLD"
    ...
    def test_exit_stop_loss(self):
        brain = TradingBrain()
        decision = brain.decide(df=pd.DataFrame(), score=0.0, has_position=True, position_pnl_pct=-0.10)
        assert decision.action == "SELL"
        assert "stop-loss" in decision.reason

    def test_exit_take_profit(self):
        brain = TradingBrain()
        decision = brain.decide(df=pd.DataFrame(), score=0.0, has_position=True, position_pnl_pct=0.20)
        assert decision.action == "SELL"
        assert "take-profit" in decision.reason

    def test_exit_sell_score(self):
        brain = TradingBrain()
        decision = brain.decide(df=pd.DataFrame(), score=-0.50, has_position=True, position_pnl_pct=0.0)
        assert decision.action == "SELL"
        assert "sell score" in decision.reason

    def test_exit_hold(self):
        brain = TradingBrain()
        decision = brain.decide(df=pd.DataFrame(), score=-0.10, has_position=True, position_pnl_pct=0.0)
        assert decision.action == "HOLD"
        assert "position still valid" in decision.reason
