    ...
    <section id="panel-backtest" className="panel flex flex-col gap-6 animate-fade-in-up w-full">
      <div className="glass-card">
        ...
        <p className="text-xs text-slate-500 mb-6">
          Capital inicial: <strong>${p.initial_capital.toLocaleString()}</strong> ·
          Comisiones: <strong>{(p.commission_pct * 100).toFixed(2)}%</strong> ·
          Deslizamiento: <strong>{(p.slippage_pct * 100).toFixed(3)}%</strong>
        </p>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {Components.metricCard('Capital Final', '$' + m.capital_final.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }), '', m.capital_final >= p.initial_capital ? 'green' : 'red')}
