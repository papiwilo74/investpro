...
def price_chart(df: pd.DataFrame, ticker: str) -> go.Figure:
    ...
    # Volumen
    vol_colors = [COLORS["vol_up"] if c >= o else COLORS["vol_down"] for c, o in zip(df["close"], df["open"])]
    fig.add_trace(
        go.Bar(
            x=df.index,
            y=df["volume"],
...
def rsi_chart(df: pd.DataFrame) -> go.Figure:
    ...
    # Zonas
    fig.add_hline(y=70, line_dash="dash", line_color=COLORS["red"], opacity=0.5, annotation_text="Sobrecompra")
    fig.add_hline(y=30, line_dash="dash", line_color=COLORS["green"], opacity=0.5, annotation_text="Sobreventa")
...
def macd_chart(df: pd.DataFrame) -> go.Figure:
    ...
    # Histograma
    hist_colors = [COLORS["green"] if v >= 0 else COLORS["red"] for v in df["macd_histogram"]]
    fig.add_trace(
        go.Bar(
            x=df.index,
            y=df["macd_histogram"],
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertext=[
                ...
                for r, v, s in zip(
                    random_ports_df["return"], random_ports_df["volatility"], random_ports_df["sharpe_ratio"]
                )
            ],
            hoverinfo="text",
        )
